...
class VQADataset(Dataset):
    def __init__(self, features_dir, allindex, dis_names, ref_names, allmos, seg_num=4, seg_len=24, feat_dim=4096):
    ...
    def __getitem__(self, index):
        ...
        tmpvideo = abs(tmpvideo[:tmp_length, :] - tmpvideo[tmp_length:, :])

        org_audio_features = np.load(self.features_dir + 'skip2_' + self.dis_names[self.allindex[index]][1] + '_res5.npy')
        ref_audio_features = np.load(self.features_dir + 'skip2_' + self.ref_names[self.allindex[index]][1] + '_res5.npy')
        tmpaudio = abs(org_audio_features - ref_audio_features)
        for j in range(seg_num):
            features[j, :, :] = tmpvideo[j * self.seg_len:(j + 1) * self.seg_len, :self.feat_dim]
...
class VSFA(nn.Module):
    ...
    def forward(self, features, afeatures):
        video_input = self.video_ann(features)
...
if __name__ == "__main__":
    parser = ArgumentParser(description='ANNAVQA FR')
    ...
    parser.add_argument('--exp_id', default='ANNAVQA_ref', type=str,
                        help='exp id (default: ANNAVQA_ref)')
    ...
    for i in range(0, len(Info['disNames'])):
        ...
        refNames.append([''.join(Info['refNames'][i, 0]), ''.join(Info['refNames'][i, 1]), Info['frameNum'][i, 0]])
    ...
    for te in range(test_epoch):
        ...
        train_dataset = VQADataset(features_dir, train_index, disNames, refNames, MOSz, seg_num, seg_len, feat_dim)
        train_loader = torch.utils.data.DataLoader(dataset=train_dataset, batch_size=args.batch_size, shuffle=True,
                                                   pin_memory=False, num_workers=0)
        valid_dataset = VQADataset(features_dir, valid_index, disNames, refNames, MOSz, seg_num, seg_len, feat_dim)
        valid_loader = torch.utils.data.DataLoader(dataset=valid_dataset, batch_size=1, shuffle=False,
                                                   pin_memory=False, num_workers=0)
        test_dataset = VQADataset(features_dir, test_index, disNames, refNames, MOSz, seg_num, seg_len, feat_dim)
